...
----print("name: "..name)
		if (name=="" or name==nil) then
		  ...
		  if (lang=="") then
		  	...
		  	words[i].snd = audio.loadSound(audioDir..lang.."_"..name ..".mp3")
		  end
		  words[i].id = i
...
function pauseSentence(button, channel )
	...
	print(#anim )
	for i = 1,#anim do
			transition.pause(anim[i]) 
	end
	print( "CAMBIO-----------" )
	audio.pause( channel )
end

function reanudeSentence(btn, channel )
	-- body
	btn.pause = false
        	...
        	if (sentence) then
            	local syncClosure = function()
       			...
       			-- fade button so it's not touchable
                    transition.to(button, { time=trans2, delay=0, alpha=.01 } ) --originally .9
                    transition.to(button, { time=trans2, delay=line.soundLength+trans1, alpha=button.alpha } ) --originally 1
                ...
                --		transIn[i]=transition.dissolve(text[i].activeText,text[i],trans2,delay2)
                		...
                		if text[i].trigger ~= nil then
                			timerStash.a = timer.performWithDelay( line[i].start, text[i].trigger)                    			        
                		end
...
--print("step 4 - play on request")
    ...
    --audio.rewind(sentence)
		...
		for i = 1,#line do
			...
			-- rather than using dissolve, which looks choppy, let's just fade in the highlight
			-- text that's sitting on top of the black text.
			table.insert(button.anim, transition.to(text[i].activeText, { delay = delay1, alpha = 1, time=trans1 } ))
			table.insert(button.anim, transition.to(text[i].activeText, { delay = delay2, alpha = 0, time=trans2 } ))
